...
function TableListPanel({ tables, selectedTableName, onSelect }: TableListPanelProps) {
  const t = useTranslations('dashboard.tableAccess');
      ...
      {tables.length > 5 && (
        <div className="border-b border-border p-2">
          <SearchInput
            ...
            ariaLabel={t('tables.filterAriaLabel')}
            debounceMs={150}
          />
        </div>
      )}
        ...
        <ScaffoldFilterAndContent>
          <ScaffoldActionsContainer>
            <div className="flex items-center gap-3">
              <span className="font-sans text-body text-fg-muted">{t('user')}</span>
              {usersLoading ? (
            ...
            {loading ? (
              ...
              >
                <EmptyState
                  ...
                  primaryAction={
                    companyId
                      ? { label: t('protectTable'), onClick: () => setProtectDialogOpen(true) }
                      : undefined
                  }
                />
              </motion.div>
