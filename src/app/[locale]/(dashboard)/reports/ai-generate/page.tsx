...
interface FriendlyError {
  key: ErrorKey;
  actionHref?: string;
  actionLabelKey?: 'browseTemplates' | 'createTemplate';
}

const NO_TEMPLATE_ERROR: FriendlyError = {
  key: 'noTemplate',
  actionHref: '/reports/templates',
  actionLabelKey: 'browseTemplates',
};

const ERROR_KEY_MAP: Record<string, FriendlyError> = {
  NO_TEMPLATE_MATCH: NO_TEMPLATE_ERROR,
  TemplateValidationError: NO_TEMPLATE_ERROR,
  REPORT_TEMPLATE_VALIDATION_ERROR: {
    key: 'noTemplatesConfigured',
    actionHref: '/reports/templates/new',
    actionLabelKey: 'createTemplate',
  },
  SSE_CONNECTION_ERROR: { key: 'connectionLost' },
  SUBMIT_ERROR: { key: 'submitFailed' },
};

const MESSAGE_MATCH_PATTERNS: Array<{ match: string } & FriendlyError> = [
  { match: 'no templates are available', ...NO_TEMPLATE_ERROR },
  { match: 'no templates found', ...NO_TEMPLATE_ERROR },
  { match: 'connection not found', key: 'connectionLost' },
  {
...
function getFriendlyError(code: string, message: string, recoverable: boolean): FriendlyError {
  if (ERROR_KEY_MAP[code]) return ERROR_KEY_MAP[code];
  const lower = message.toLowerCase();
  const matched = MESSAGE_MATCH_PATTERNS.find((p) => lower.includes(p.match));
  if (matched) return matched;
  return { key: recoverable ? 'defaultRecoverable' : 'defaultFailed' };
}
...
export default function AIReportChatPage() {
  ...
  const te = useTranslations('dashboard.reports.aiGenerate.errors');
              ...
              >
                ...
                <AnimatePresence>
                  {state.status === 'error' && state.error && (() => {
                    const errInfo = getFriendlyError(
                      state.error.code,
                      state.error.message,
                      state.error.recoverable,
                    );
                    const titleKey = `${errInfo.key}Title` as const;
                    const hintKey = `${errInfo.key}Hint` as const;
                    return (
                      <motion.div
                        variants={fadeUp}
                        initial="hidden"
                        ...
                        role="alert"
                      >
                        <p className="font-sans text-button text-error">{te(titleKey)}</p>
                        <p className="font-sans text-body-sm text-subtle">{te(hintKey)}</p>
                        {errInfo.actionHref && errInfo.actionLabelKey && (
                          <Link
                            href={errInfo.actionHref}
                            className="inline-block font-sans text-body-sm text-accent hover:underline mt-1"
                          >
                            {te(errInfo.actionLabelKey)} →
                          </Link>
                        )}
                      </motion.div>
                    );
