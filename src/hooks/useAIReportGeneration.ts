...
type Action =
  ...
  | { type: 'SET_TEMPLATE_SELECTION'; data: TemplateSelectionResult }
  | { type: 'SET_QUERY_ADAPTATION'; data: QueryAdaptationResult }
  | { type: 'SET_SECTION_RESULT'; sectionIndex: number; title: string; key_findings?: string[]; narrative?: string }
  | { type: 'SET_EXECUTIVE_SUMMARY'; summary: string; keyMetrics: AIReportPipelineState['keyMetrics']; recommendations: string[] }
  | { type: 'SET_ERROR'; code: string; message: string; stepName?: string; recoverable: boolean }
  | { type: 'COMPLETE'; durationMs: number };

// ============== State ==============

function createInitialState(): AIReportPipelineState {
  return {
    status: 'idle',
    steps: AI_REPORT_PIPELINE_STEPS.map((s) => ({
      ...s,
...
function reducer(state: AIReportPipelineState, action: Action): AIReportPipelineState {
  switch (action.type) {
    case 'RESET':
    ...
    case 'SET_TEMPLATE_SELECTION':
      return { ...state, templateSelection: action.data };

    case 'SET_QUERY_ADAPTATION':
      return { ...state, queryAdaptation: action.data };
...
export function useAIReportGeneration() {
  ...
  const generate = useCallback(async (
    ...
    try {
      const { task_id, generation_id } = await aiGenerateReport(
        query, companyId, connectionId, title, language
      );
        ...
        (eventType: SSEEventType, data: Record<string, unknown>) => {
          switch (eventType) {
            ...
            case 'intermediate_result': {
              ...
              if (resultType === 'template_selection') {
                dispatch({
                  type: 'SET_TEMPLATE_SELECTION',
                  data: resultData as unknown as TemplateSelectionResult,
                });
              } else if (resultType === 'query_adaptation') {
                dispatch({
                  type: 'SET_QUERY_ADAPTATION',
                  data: resultData as unknown as QueryAdaptationResult,
                });
              } else if (resultType === 'section_analysis') {
                dispatch({
                ...
                dispatch({
                  ...
                  keyMetrics: (resultData.key_metrics || []) as AIReportPipelineState['keyMetrics'],
      ...
      const message = err instanceof Error ? err.message : 'Failed to start AI report generation';
      dispatch({
        type: 'SET_ERROR',
        code: 'SUBMIT_ERROR',
        message,
